...
'use strict';var _notify=require('../utils/notify.js'),settings=require('../lib/settings.js'),modal=require('../utils/modal.js'),options=require('../utils/options.js'),myModule={};myModule.id='dj-notification',myModule.moduleName='DJ Notification',myModule.description='Notification when you are coming up to be the DJ',myModule.category='General',myModule.extraIcon='pencil';var savePosition=function(){var a=parseInt($('.dp-modal textarea').val());isNaN(a)?options.saveOption('custom','dj_notification',2):options.saveOption('custom','dj_notification',a)};myModule.djNotificationCheck=function(a){if(!(2<a.startTime)){var b=parseInt($('.dj-position').text());isNaN(b)||b!==settings.custom.dj_notification||((0,_notify.showNotification)({title:'DJ Alert!',content:'You will be DJing shortly! Make sure your song is set!',ignoreActiveTab:!0,wait:1e4}),Dubtrack.room.chat.mentionChatSound.play())}},myModule.turnOn=function(){Dubtrack.Events.bind('realtime:room_playlist-update',this.djNotificationCheck)},myModule.extra=function(){modal.create({title:'DJ Notification',content:'Position in queue to notify at',value:settings.custom.dj_notification||'',placeholder:'2',maxlength:'2',confirmCallback:savePosition})},myModule.turnOff=function(){Dubtrack.Events.unbind('realtime:room_playlist-update',this.djNotificationCheck)},module.exports=myModule;
