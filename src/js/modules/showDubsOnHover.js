...
dubshover.grabInfoWarning = function () {
  ...
  modal.create({
    ...
    content:
      'Please note that this feature is currently still in development. We are waiting on the ability to pull grab vote information from Dubtrack on load. Until then the only grabs you will be able to see are those you are present in the room for.',
  });
};
        ...
        html += `
          <li class="preview-dubinfo-item users-previews dubplus-updubs-hover">
            <div class="dubinfo-image">
              <img src="${apiBase}/user/${val.userid}/image">
            </div>
...
dubshover.dubUserLeaveWatcher = function (e) {
  ...
  if (
    $.grep(window.dubplus.dubs.upDubs, function (el) {
  ...
  if (
    $.grep(window.dubplus.dubs.downDubs, function (el) {
      return el.userid === e.user._id;
    }).length > 0
  ) {
  ...
  if (
    $.grep(window.dubplus.dubs.grabs, function (el) {
      return el.userid === e.user._id;
    }).length > 0
  ) {
...
dubshover.dubWatcher = function (e) {
  if (e.dubtype === 'updub') {
    ...
    if (
      ...
      userIsAtLeastMod(QueUp.session.id)
    ) {
  ...
  var msSinceSongStart =
    new Date() - new Date(QueUp.room.player.activeSong.attributes.song.played);
  if (msSinceSongStart < 1000) {
    return;
  }

  if (
    window.dubplus.dubs.upDubs.length !==
    QueUp.room.player.activeSong.attributes.song.updubs
  ) {
    // console.log("Updubs don't match, reset! Song started ", msSinceSongStart, "ms ago!");
    this.resetDubs();
  } else if (
    userIsAtLeastMod(QueUp.session.id) &&
    window.dubplus.dubs.downDubs.length !==
      QueUp.room.player.activeSong.attributes.song.downdubs
  ) {
...
dubshover.resetDubs = function () {
  ...
  var dubsURL = `${apiBase}/room/${QueUp.room.model.id}/playlist/active/dubs`;
  ...
  $.getJSON(dubsURL, (response) => {
    response.data.upDubs.forEach((e) => {
      ...
      var username;
      if (
        !QueUp.room.users.collection.findWhere({ userid: e.userid }) ||
        !QueUp.room.users.collection.findWhere({ userid: e.userid }).attributes
      ) {
        $.getJSON(`${apiBase}/user/${e.userid}`, function (response) {
          if (response && response.userinfo) {
            username = response.userinfo.username;
          }
        });
      } else {
        username = QueUp.room.users.collection.findWhere({ userid: e.userid })
