...
`);
  ...
  async uploadExtension() {
    ...
    const url = `https://chromewebstore.googleapis.com/upload/v2/${this.itemPath}:upload`;
  ...
  async publishExtension() {
    ...
    const url = `https://chromewebstore.googleapis.com/v2/${this.itemPath}:publish`;
   ...
   */
  async getExtensionInfo({ quiet = false } = {}) {
    // curl -H "Authorization: Bearer $TOKEN"
    // -X GET
    ...
    const url = `https://chromewebstore.googleapis.com/v2/${this.itemPath}:fetchStatus`;
  ...
  async waitForUpload() {
    ...
    for (let attempt = 1; attempt <= POLL_MAX_ATTEMPTS; attempt++) {
      ...
      const { lastAsyncUploadState } = await this.getExtensionInfo({
        quiet: true,
      });
   ...
   */
  async handleUploadState(uploadResponse) {
  ...
  async run() {
    try {
      ...
      const { publishedVersion } = this.reviewCurrentStatus(info);
      this.checkVersion(publishedVersion);

      // Upload the new package.
      const uploadResponse = await this.uploadExtension();
      await this.handleUploadState(uploadResponse);

      // Submit it.
      const { state, warningInfo } = await this.publishExtension();
      ...
      this.reportState(state, 'Publish result');

      // Confirm what the store thinks the state is now.
      console.log('\n📊 Final extension status:');
      const postPublishInfo = await this.getExtensionInfo({ quiet: true });
      const submitted = postPublishInfo.submittedItemRevisionStatus;
      this.reportState(submitted?.state, 'Submitted revision');
